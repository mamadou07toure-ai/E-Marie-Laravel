...
<style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .top-band { background: #065f46; padding: 0 40px; height: 90px; position: relative; overflow: hidden; }
    .top-band::after { content: ''; position: absolute; top: 0; right: 0; width: 220px; height: 90px; background: #10b981; clip-path: polygon(40px 0, 100% 0, 100% 100%, 0 100%); }
    ...
    .title-band { background: #f0fdf4; border-bottom: 1px solid #d1fae5; padding: 16px 40px; }
    .doc-main-title { font-size: 19px; font-weight: 900; color: #065f46; text-transform: uppercase; letter-spacing: 1px; }
    .doc-main-sub   { font-size: 8px; font-weight: 700; color: #10b981; text-transform: uppercase; letter-spacing: 2px; margin-top: 4px; }

    .body-wrap { padding: 20px 40px 200px 40px; }

    .cert-paragraph { font-size: 9.5px; color: #374151; line-height: 1.75; border-left: 3px solid #10b981; padding-left: 12px; margin-bottom: 20px; font-style: italic; }

    .section { margin-bottom: 18px; }
    .section-head-left { background: #065f46; color: white; font-size: 8px; font-weight: 900; text-transform: uppercase; letter-spacing: 1.5px; padding: 5px 14px; display: block; }

    .data-grid { width: 100%; border-collapse: collapse; }
    .data-grid tr { border-bottom: 1px solid #f1f5f9; }
    .data-grid tr:nth-child(even) td { background: #f9fffe; }
    .data-grid .k { width: 36%; font-size: 8px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: .3px; padding: 7px 14px; border-right: 2px solid #10b981; vertical-align: middle; }
    .data-grid .v { font-size: 11px; font-weight: 900; color: #111827; padding: 7px 14px; vertical-align: middle; }

    .validity-block { background: #f0fdf4; border: 1px solid #a7f3d0; padding: 10px 14px; margin-top: 18px; text-align: center; font-size: 9px; font-weight: 700; color: #065f46; }

    .footer-fixed { position: fixed; bottom: 0; left: 0; right: 0; height: 185px; background: #fff; border-top: 1px solid #d1fae5; padding: 14px 40px 0 40px; }
    ...
    .qr-img-box { border: 1px solid #d1fae5; padding: 3px; display: inline-block; background: #fff; }
    .qr-caption { font-size: 7px; color: #9ca3af; margin-top: 4px; font-weight: 700; text-transform: uppercase; }
    .uuid-mono  { font-size: 6.5px; color: #065f46; font-family: monospace; margin-top: 2px; }

    .seal-ring { width: 72px; height: 72px; border-radius: 50%; border: 2px solid #065f46; margin: 0 auto 6px; text-align: center; padding-top: 12px; font-size: 7px; font-weight: 900; color: #065f46; text-transform: uppercase; line-height: 1.4; }
    .seal-inner { border: 1px dashed #065f46; border-radius: 50%; margin: 3px; padding: 6px 2px 2px; height: 60px; }

    .foot-legal { font-size: 7px; color: #9ca3af; text-align: center; text-transform: uppercase; letter-spacing: .5px; margin-top: 6px; border-top: 1px dashed #d1fae5; padding-top: 6px; }
    .sign-label { font-size: 8.5px; font-weight: 900; color: #065f46; text-transform: uppercase; }
    ...
    .bottom-accent { position: fixed; bottom: 0; left: 0; right: 0; height: 5px; background: linear-gradient(90deg, #065f46, #10b981, #34d399); }
</style>
</head>
<body>
@php
...
<div class="footer-fixed">
    <div class="footer-inner">
        ...
        <div class="foot-sign">
            <div class="sign-label">Le Maire de la Commune</div>
    ...
    <div class="top-band-content">
        ...
        <div class="top-band-right">
            <div class="doc-ref-box">
                <div class="ref-label">Certificat N°</div>
                <div class="ref-value">RES-{{ str_pad($demande->id, 6, '0', STR_PAD_LEFT) }}</div>
...
<div class="title-band">
    <div class="doc-main-title">Certificat de Résidence</div>
    <div class="doc-main-sub">Attestation officielle de domicile &mdash; {{ now()->format('Y') }}</div>
</div>

<div class="body-wrap">
    <p class="cert-paragraph">
        Le Maire de la Commune de Kaloum, après vérification des pièces justificatives fournies, certifie que la personne identifiée ci-dessous réside effectivement et de manière habituelle dans notre juridiction communale.
    </p>

    <div class="section">
        <span class="section-head-left">Identité du Résident</span>
        <table class="data-grid">
            ...
            <tr><td class="k">Profession</td><td class="v">{{ mb_convert_case($d['profession'] ?? '—', MB_CASE_TITLE, 'UTF-8') }}</td></tr>
        </table>
    </div>

    <div class="section">
        <span class="section-head-left">Informations de Domiciliation</span>
        <table class="data-grid">
            <tr><td class="k">Quartier / District</td><td class="v">{{ strtoupper($d['quartier'] ?? '—') }}</td></tr>
            <tr><td class="k">Adresse complète</td><td class="v">{{ $d['adresse'] ?? '—' }}</td></tr>
            <tr><td class="k">Durée de résidence</td><td class="v">{{ $d['duree_residence'] ?? 'Non spécifiée' }}</td></tr>
    ...
    <div class="validity-block">
        ...
        Il est valable pour une durée de <strong>TROIS (3) MOIS</strong> à compter de sa date de signature.
    </div>
</div>
</body>
</html>
