...
export default function Messages({ auth }) {
    ...
    return (
        <AgentLayout title="Messagerie">
            <Head title="Messagerie - Espace Agent" />
            <Toaster position="top-right" richColors />

            <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden" style={{ height: 'calc(100vh - 12rem)' }}>
                <div className="flex h-full">
                    ...
                    <div className={`w-full md:w-80 lg:w-96 border-r border-slate-100 flex flex-col ${selectedUserId ? 'hidden md:flex' : 'flex'}`}>
                        <div className="p-5 border-b border-slate-50 bg-slate-50/30">
                            <h2 className="text-xs font-black text-slate-900 uppercase tracking-[0.2em] mb-4">Messagerie Agent</h2>
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full bg-white rounded-xl pl-10 pr-4 py-2.5 text-[11px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-indigo-500/10 border border-slate-100 focus:border-indigo-500 transition-all placeholder:text-slate-300"
                                />
                            </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto">
                            {loadingConvs ? (
                                ...
                                <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
                                    <div className="w-14 h-14 bg-slate-50 rounded-3xl flex items-center justify-center text-slate-200 mb-4 border border-slate-100 shadow-inner">
                                        <MessageSquare size={28} />
                                    </div>
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Aucune conversation</p>
                                    <p className="text-[10px] text-slate-300 font-bold mt-1">Les citoyens vous contacteront depuis leurs dossiers</p>
                                </div>
                            ) : (
                                filteredConversations.map(conv => (
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-3 px-5 py-4 hover:bg-indigo-50/20 transition-all text-left border-b border-slate-50 group ${selectedUserId === conv.user_id ? 'bg-indigo-50/30' : ''}`}
                                    >
                                        <div className="w-11 h-11 rounded-2xl bg-slate-100 border border-slate-200 text-slate-400 font-black flex items-center justify-center text-sm shrink-0 group-hover:bg-indigo-600 group-hover:text-white group-hover:border-indigo-600 transition-all">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center justify-between mt-0.5">
                                                ...
                                                {conv.unread > 0 && (
                                                    <span className="ml-2 w-5 h-5 bg-rose-500 text-white rounded-full text-[9px] font-black flex items-center justify-center shrink-0">
                                                        {conv.unread}
                                                    </span>
                                                )}
                                            </div>
                        ...
                        {selectedUserId && selectedUser ? (
                            <>
                                <div className="flex items-center gap-4 px-6 py-4 border-b border-slate-100 bg-slate-50/30">
                                    ...
                                    <div className="w-10 h-10 rounded-xl bg-slate-100 border border-slate-200 text-slate-500 font-black flex items-center justify-center text-sm">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-1.5 mt-0.5">
                                            ...
                                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Citoyen</p>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        messages.map(msg => {
                                            ...
                                            return (
                                                <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                                    <div className={`group max-w-xs lg:max-w-md relative flex flex-col gap-1 ${isMe ? 'items-end' : 'items-start'}`}>
                                ...
                                {editingMessage && (
                                    <div className="px-6 py-2 bg-amber-50 border-t border-amber-100 flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <Edit2 size={12} className="text-amber-600" />
                                            <span className="text-[10px] font-black text-amber-600 uppercase tracking-widest">Modification en cours</span>
                                        </div>
                                        <button onClick={cancelEdit} className="text-amber-400 hover:text-amber-600">
                                            <X size={14} />
                                        </button>
                                    </div>
                                )}
                                <form onSubmit={handleSend} className="flex items-center gap-3 px-6 py-4 border-t border-slate-100 bg-white">
                                    ...
                                    <button
                                        ...
                                        className="w-11 h-11 bg-slate-900 text-white rounded-xl flex items-center justify-center hover:bg-indigo-600 transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed active:scale-95"
                                    >
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center text-center p-10">
                                ...
                                <h3 className="text-lg font-black text-slate-900 tracking-tight mb-2">Messagerie Agent</h3>
                                <p className="text-slate-400 text-sm font-medium max-w-xs">
                                    Sélectionnez une conversation pour répondre à un citoyen.
                                </p>
                            </div>
                        )}
                    </div>
